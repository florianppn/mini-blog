...
\setlength{\arrayrulewidth}{0.4pt}
\begin{longtable}{|>{\raggedright\arraybackslash\bfseries}p{1.45cm}|>{\raggedright\arraybackslash\ttfamily}p{4.2cm}|>{\raggedright\arraybackslash}p{2.5cm}|>{\raggedright\arraybackslash}p{6.15cm}|}
\caption{Répertoire complet des endpoints de l'API REST Mini-Blog} \label{tab:api-endpoints} \\
\hline
\rowcolor{gray!75}
\color{white}\textbf{Méthode} & \color{white}\textbf{Chemin (URI)} & \color{white}\textbf{Accès / Rôle} & \color{white}\textbf{Rôle et description fonctionnelle} \\
\hline
\endfirsthead
...
\hline \multicolumn{4}{|r|}{{Suite page suivante...}} \\ \hline
\endfoot
...
\subsection*{Tableau A : Matrice des Tests Unitaires (39 tests)}
...
\newpage
\subsection*{Tableau B : Matrice des Tests d'Intégration (5 tests)}
