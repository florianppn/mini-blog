...
\begin{tabularx}{\textwidth}{|>{\bfseries}p{3.2cm}|p{2.6cm}|X|X|}
\hline
\rowcolor{gray!75}
...
\hline
\rowcolor{gray!10}
Brouillons (\texttt{DRAFT}) & Inaccessibles & Édition, suppression \& soumission & Inaccessibles (privés à l'auteur) \\
\hline
En attente (\texttt{PENDING}) & Inaccessibles & Consultation \& annulation de soumission & Examen, validation \& renvoi en brouillon \\
\hline
\rowcolor{gray!10}
Commentaires sur publiés & Lecture seule & Ajout, édition \& suppression de ses écrits & Ajout \& modération de tous les commentaires \\
\hline
Commentaires sur non publiés & \centering$\times$ Interdit & \centering$\times$ Interdit & \centering\arraybackslash$\times$ Interdit \\
\hline
\end{tabularx}
\end{table}
...
\paragraph{Options :}
\begin{itemize}
    \item[$\boxtimes$] Boîte de dialogue modale réutilisable (\texttt{ConfirmDialog}) réactive, avec charte graphique par sévérité (\textit{danger}, \textit{warning}, \textit{info}) et accessibilité clavier (\textit{Échap}, \textit{Entrée}).
...
\hline
\textbf{Action / Ressource} & \textbf{Visiteur} & \textbf{Auteur (\texttt{ROLE\_USER})} & \textbf{Admin (\texttt{ROLE\_ADMIN})} \\
\hline
Consulter les articles publiés & $\checkmark$ & $\checkmark$ & $\times$ (Redirection \texttt{/admin}) \\
Rechercher des articles & $\checkmark$ & $\checkmark$ & $\times$ \\
Commenter un article publié & $\times$ & $\checkmark$ & $\times$ (Réservé aux lecteurs) \\
Rédiger un article (\texttt{POST}) & $\times$ & $\checkmark$ (\texttt{DRAFT}) & $\times$ (403 Forbidden) \\
Modifier le texte (\texttt{PUT}) & $\times$ & $\checkmark$ (Auteur si \texttt{DRAFT}) & $\times$ (403 Forbidden) \\
Soumettre à validation & $\times$ & $\checkmark$ (Auteur seul) & $\times$ \\
Valider et publier & $\times$ & $\times$ & $\checkmark$ (\texttt{/admin}) \\
Renvoyer en brouillon & $\times$ & $\times$ & $\checkmark$ (\texttt{/admin}) \\
Dépublier un article & $\times$ & $\times$ & $\checkmark$ (\texttt{/admin}) \\
Supprimer un article & $\times$ & $\checkmark$ (Ses brouillons) & $\checkmark$ (\texttt{/admin}) \\
\hline
\end{tabular}
...
\begin{itemize}
    ...
    \item \textbf{Couverture de tests} : Suite automatisée de 24 tests unitaires et d'intégration validant 100\% de la matrice des droits et des règles d'étanchéité éditoriale.
